...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_att=False, output_qkv=False):
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(hidden_states)
        mixed_value_layer = self.value(hidden_states)
        ...
        if output_qkv:
            return context_layer, query_layer, key_layer, value_layer
        return context_layer
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, output_att=False, output_qkv=False):
        if output_att:
            self_output, layer_att = self.self(
                input_tensor, attention_mask, output_att=True)
        elif output_qkv:
            self_output, query_layer, key_layer, value_layer = self.self(
                input_tensor, attention_mask, output_qkv=True)
        else:
        ...
        if output_qkv:
            return attention_output, query_layer, key_layer, value_layer
    ...
    def forward(self, hidden_states, attention_mask, output_att=False, output_qkv=False):
        if output_att:
            attention_output, layer_att = self.attention(
                hidden_states, attention_mask, output_att=True)
        elif output_qkv:
            attention_output, query_layer, key_layer, value_layer = self.attention(
                hidden_states, attention_mask, output_qkv=True)
        else:
        ...
        if output_qkv:
            return layer_output, query_layer, key_layer, value_layer
        return layer_output
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, 
                output_att=False, output_qkv=False, layer_index=-1):
        all_encoder_layers = []
        all_encoder_atts = []
        if output_qkv and layer_index < 0:
            layer_index += len(self.layer)
        for i, layer_module in enumerate(self.layer):
            ...
            elif output_qkv and i == layer_index:
                hidden_states, query_layer, key_layer, value_layer = layer_module(
                    hidden_states, attention_mask, output_qkv=True)
            else:
        ...
        if output_qkv:
            return all_encoder_layers, query_layer, key_layer, value_layer
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if not hasattr(model, 'bert') and any(s.startswith('bert.') for s in state_dict.keys()):
            start_prefix = 'bert.'
        if hasattr(model, 'bert') and (not any(s.startswith('bert.') for s in state_dict.keys())):
            model_to_load = getattr(model, 'bert')
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                output_all_encoded_layers=True, output_att=False, output_qkv=False, layer_index=-1):
        ...
        if output_att:
            encoded_layers, layer_atts = self.encoder(
                embedding_output, extended_attention_mask, output_att=True)
        elif output_qkv:
            encoded_layers, query_layer, key_layer, value_layer = self.encoder(
                embedding_output, extended_attention_mask, 
                output_qkv=True, layer_index=layer_index)
        else:
        ...
        if output_att:
            return encoded_layers, pooled_output, layer_atts
        if output_qkv:
            return encoded_layers, pooled_output, query_layer, key_layer, value_layer
...
class TinyBertForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, 
                masked_lm_labels=None, next_sentence_label=None):
        sequence_output, pooled_output, att_output = self.bert(
            input_ids, token_type_ids, attention_mask, output_att=True)
        tmp = []
        for s_id, sequence_layer in enumerate(sequence_output):
            tmp.append(self.fit_dense(sequence_layer))
        sequence_output = tmp

        return sequence_output, att_output
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if is_student:
            for s_id, sequence_layer in enumerate(sequence_output):
                tmp.append(self.fit_dense(sequence_layer))
            sequence_output = tmp
        return logits, sequence_output, att_output


class MiniLMForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, num_labels):
        super(MiniLMForSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                labels=None, layer_index=-1):
        sequence_output, pooled_output, query_layer, key_layer, value_layer = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False,
            output_qkv=True, layer_index=-1)
        
        logits = self.classifier(torch.relu(pooled_output))
        return logits, query_layer, key_layer, value_layer


class DistillBertForSemantics(BertPreTrainedModel):
    def __init__(self, config):
        super(DistillBertForSemantics, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, a_input_ids, b_input_ids, a_token_type_ids=None, b_token_type_ids=None,
        ...
        a_pooled_output = F.normalize(a_pooled_output, p=2, dim=-1)
        b_pooled_output = F.normalize(b_pooled_output, p=2, dim=-1)
        
        similarity = torch.sum(a_pooled_output * b_pooled_output, dim=-1)
        return similarity
